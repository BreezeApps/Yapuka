...
export function ListContainer({ boardId, reloadList, setReloadList, blur }: { boardId: number, reloadList: boolean, setReloadList: (reload: boolean) => void, blur: boolean }) {
    ...
    <div className="p-6">
      ...
      <div>
        {collections
          ...
          <div
            key={collection.id}
            className="relative flex-col rounded-lg bg-gray-300 shadow-sm border border-slate-200 min-w-[240px] gap-1 p-1.5 list float-left inline m-3"
            >
            <h3
              className={`rounded capitalize text-center p-2 font-bold`}
              style={{ 
                backgroundColor: collection.color ? collection.color.toString(): '#d1d5dc'
              }}
            >
              {collection.name}
              ...
              <div className="overflow-hidden">
                ...
                <ModalForm blur={blur} type="task" collectionId={collection.id.toString()} onCreate={handleCreateTask} />
              </div>
            </h3>
            <div>
              {tasks
                ...
                .map((task) => (
                  <div
                    key={task.id}
                    id={`task-${task.id}`}
                    className="text-slate-800 flex w-full items-center rounded-md transition-all hover:bg-slate-100 focus:bg-slate-100 active:bg-slate-100"
                    role="button"
                  >
                    {task.name}
