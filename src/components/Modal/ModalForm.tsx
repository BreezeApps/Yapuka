...
type ModalFormProps = {
  ...
  collectionId?: string
  onCreate: (name: string, description?: string, date?: string, color?: string, collection_id?: string) => void;
};

export function ModalForm({ type, collectionId, onCreate, blur }: ModalFormProps) {
  ...
  const handleSubmit = () => {
    if (!name) return;
    onCreate(name, description, date, color, collectionId);
    setName("");
    setDescription("");
    setDate("")
    setColor(undefined);
  ...
  return (
    <Dialog.Root>
      ...
      <Dialog.Portal>
        <Dialog.Overlay className={`fixed inset-0 ${ blur === true ? "backdrop-blur-[20px]" : "bg-black/50"}`} />
